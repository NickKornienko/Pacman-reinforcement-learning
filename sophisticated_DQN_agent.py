...
class SophisticatedAgent:
    def __init__(self, state_shape):
        ...
        # Networks
        self.policy_net = SophisticatedNetwork(state_shape[:2], self.action_size).to(self.device)
        self.target_net = SophisticatedNetwork(state_shape[:2], self.action_size).to(self.device)
    ...
    def optimize(self):
        ...
        state_batch = torch.FloatTensor(np.array(batch[0])).contiguous().to(self.device)
        action_batch = torch.LongTensor(batch[1]).unsqueeze(1).to(self.device)
        reward_batch = torch.FloatTensor(batch[2]).to(self.device)
        next_state_batch = torch.FloatTensor(np.array(batch[3])).contiguous().to(self.device)
        ...
        try:
            for episode in range(num_episodes):
                ...
                for step in range(500):
                    ...
                    # Store transition
                    self.memory.push(state, action, shaped_reward, next_state, done or truncated)
                    state = next_state

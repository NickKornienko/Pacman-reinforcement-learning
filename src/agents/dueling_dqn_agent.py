"""
Dueling DQN implementation with move tracking
...
class DuelingDQNNetwork(nn.Module):
    """Neural network for Dueling DQN with separate value and advantage streams"""
...
class DuelingDQNAgent(BaseAgent):
    """Dueling DQN Agent with move tracking"""
    def __init__(self, state_shape, action_size, device='cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        # Networks
        self.policy_net = DuelingDQNNetwork(state_shape, action_size).to(device)
        self.target_net = DuelingDQNNetwork(state_shape, action_size).to(device)
    ...
    def save(self, path):
        ...
        self.save_metrics(metrics_path, 'DuelingDQN')
